""" Learning script """
# Learning how to compare objects

class Book:
    """ Book class. """
    def __init__(self, title, author, price):
        super().__init__()
        self.title = title
        self.author = author
        self.price = price

    # TODO: the __eq__ method checks for equality between two objects
    def __eq__(self, value):
        # Checking if something is not a book and raising an error
        if not isinstance(value, Book):
            raise ValueError('Can not compare book to a non-book.')

        # Checking that the attributes are the same for two objects, this will
        # be the equality
        return (self.title == value.title and
                self.author == value.author and
                self.price == value.price)

    # TODO: the __ge__ establishes >= relationship with other object
    def __ge__(self, value):
        # Checking if something is not a book and raising an error
        if not isinstance(value, Book):
            raise ValueError('Can not compare book to a non-book.')

        return self.price >= value.price


    # TODO: the __lt__ establishes < relationship with another object
    def __lt__(self, value):
        # Checking if something is not a book and raising an error
        if not isinstance(value, Book):
            raise ValueError('Can not compare book to a non-book.')

        return self.price < value.price


b1 = Book('War and Peace', 'Leo Tolstoy', 39.95)
b2 = Book('The Catcher in the Rye', 'JD Salinger', 29.95)
b3 = Book('War and Peace', 'Leo Tolstoy', 39.95)
b4 = Book('To Kill a Mockingbird', 'Harper Lee', 24.95)

# TODO: Check for equality
# print(b1 == b3)
# print(b1 == b2)
# print(b1 == 42)

# TODO: Check for greater and lesser value
# print(b2 >= b1)
# print(b2 < b1)

# TODO: Now we can sort them too
books = [b1, b2, b3, b4]
books.sort()
print([book.title for book in books])
